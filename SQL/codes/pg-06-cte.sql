...
-- 1단계: 평균 주문금액 계산
WITH avg_order AS (SELECT AVG(amount) as avg_amount	FROM orders)
-- 2단계: 평균보다 큰 주문 찾기
...
-- 1. 고객별 총 구매금액 AS customer_total CTE [customer_id, total_purchase, order_count]
-- 2. 구매 금액 기준을 계산
...
rank_info AS (
    SELECT COUNT(*) AS total_customers FROM customer_ranked
),
classified_customers AS (
    SELECT 
        ...
        c.order_count,
        CASE
            WHEN c.rnk <= CEIL(r.total_customers * 0.2) THEN 'VIP'
            WHEN c.total_purchase >= (SELECT AVG(total_purchase) FROM customer_total) THEN '일반'
            ELSE '신규'
        END AS 등급
    FROM customer_ranked c
    CROSS JOIN rank_info r
)
SELECT
    등급,
    COUNT(*) AS 등급별회원수,
    SUM(total_purchase) AS 등급별구매액총합,
    ROUND(AVG(order_count),2) AS 등급별평균주문수
